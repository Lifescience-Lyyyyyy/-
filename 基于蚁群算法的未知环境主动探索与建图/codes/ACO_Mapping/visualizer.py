...
class Visualizer:
    ...
    def draw_robot_path(self, path_taken_visual): 
        ...
        for i in range(len(path_taken_visual) - 1):
            ...
            if abs(start_r - end_r) <=1 and abs(start_c - end_c) <=1 : # Check adjacency
    ...
    def draw_ant_paths_iteration(self, robot_pos, known_map, ant_paths_this_iter, environment, iter_num, total_iters):
        ...
        self.draw_text(f"ACO Ant Iteration: {iter_num + 1}/{total_iters}", (10, self.screen_height - 30), color=BLACK) # BLACK is now defined
        self.update_display()
        pygame.time.wait(150) 

    def draw_text(self, text, position, color=BLACK, small=False): # BLACK is now defined at module level
