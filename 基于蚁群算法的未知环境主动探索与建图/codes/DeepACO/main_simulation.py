...
from planners.structuredspiral import StructuredSpiralPlanner
...
DEFAULT_PLANNERS_LIST = ["FBE", "ACO", "IGE", "URE", "StructuredSpiral"]
...
def run_simulation(
    ...
    elif planner_type_arg == "IGE": planner = IGEPlanner(env, 
                         robot_sensor_range_arg, 
                         frontier_clustering_distance=2) # 最大距离2个单元格算一个簇
    elif planner_type_arg == "URE": planner = UREPlanner(env, robot_sensor_range_arg)
    elif planner_type_arg == "StructuredSpiral": 
        planner = StructuredSpiralPlanner(env, robot_sensor_range_arg)
...
if __name__ == "__main__":
    ...
    if args.max_steps_override is not None:
        ...
        if effective_width <= 0 or effective_height <= 0 or current_sensor_range <= 0:
            ...
            term2 = effective_width - 2*term1_denominator
            calculated_steps = int((term1 + term2) * (2 * current_obs_perc_iter + 1)) 
            current_max_steps_calculated = max(200, min(calculated_steps, 20000)) 
