...
class ACOPlanner(BasePlanner):
    def __init__(self, environment, n_ants=25, n_iterations=40,
        ...
        self.pheromones = {}; # 注意：这里没有 current_path_to_target 了
    ...
    def plan_next_action(self, robot_pos, known_map, **kwargs):
        ...
        for iteration in range(self.n_iterations):
            ...
            for _ in range(self.n_ants): # Renamed ant_k to _
                ...
                if random.random() < self.q0: # Exploit
                    ...
                    if prob_sum <= 1e-9: # If no positive scores or sum is tiny
                        if reachable_frontiers_info:
                             # Fallback: pick best heuristic, or random if all heuristics are also bad
                        ...
                        if chosen_f_info is None and positive_score_candidates: # Should be rare
                            chosen_f_info = positive_score_candidates[-1]['frontier_info']
                
                if chosen_f_info is None and reachable_frontiers_info: # Final safety net
                    chosen_f_info = random.choice(reachable_frontiers_info)
                ...
                if chosen_f_info:
                    ...
                    if self.visualize_ants_callback: # Only append path if viz is active
        ...
        for f_info in reachable_frontiers_info:
            ...
            eta_loop = f_info['heuristic'] # Use the pre-calculated heuristic (which includes LoS in IG)
            tau_loop = self.pheromones.get(f_pos_loop, initial_pheromone_value)
