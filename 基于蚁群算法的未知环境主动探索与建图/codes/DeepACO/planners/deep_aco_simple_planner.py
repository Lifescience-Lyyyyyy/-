...
class Actor(nn.Module):
    ...
    def forward(self, state_features_batch):
        ...
        x = F.relu(self.fc2(x))
        return F.softplus(self.fc3(x)) + 1e-6 # Ensure positive eta
...
class DeepACOSimplePlanner(BasePlanner):
    def __init__(self, environment,
                 ...
                 alpha=1.0, beta=1.5, # Note: beta is now beta_nn_heuristic
                 ...
                 ppo_batch_size=64, 
                 is_training=True,  # Flag to switch between training and evaluation
        ...
        # NN Inputs: [IG, Cost, Explored%, LocalIGSum] -> 4 features
        self.actor = Actor(num_inputs=4).to(self.device)
        # NN Inputs: [Explored%, NumFrontiers_norm] -> 2 features
    ...
    def plan_next_action(self, robot_pos, known_map, **kwargs):
        # ... (Step 1: Feature Extraction and Initial Heuristic Calculation - same as before) ...
        ...
        # --- Step 2: Run full ACO iterations with SOFTMAX selection ---
        for _ in range(self.n_iterations):
            ant_choices_this_iter = []
            
            # Calculate scores for all frontiers first
            ...
            # *** MODIFICATION: Use Softmax for probability distribution ***
            scores_tensor = torch.tensor(scores_for_selection, dtype=torch.float32).to(self.device)
            # Apply temperature to scores (logits)
            ...
            # It's possible for softmax to result in NaN if scores are too large, or for sum to be not exactly 1.
            # Add a check for robustness.
            if np.isnan(probabilities).any() or abs(probabilities.sum() - 1.0) > 1e-6:
                # Fallback to uniform random choice if probabilities are invalid
                # print("Warning: Softmax produced invalid probabilities. Falling back to uniform choice.")
                ant_choices_indices = np.random.choice(len(reachable_frontiers_info), size=self.n_ants)
            else:
                # Each ant makes a choice based on the softmax probability distribution
        ...
        # --- Step 3: Final Robot Action Selection (can also use Softmax) ---
        final_scores = []
        for f_info in reachable_frontiers_info:
        ...
        # The robot's final choice is also made using a softmax distribution on the refined scores
        final_action_probs_dist = Categorical(logits=final_scores_tensor / self.aco_softmax_temperature) # Use temp here too
        
        if self.is_training:
            chosen_action_index = final_action_probs_dist.sample()
            
            # For PPO, the log_prob must be from the OLD policy's direct output (eta_nn_values)
            # This correctly attributes reward to the network's output, not the ACO process.
            policy_dist_for_logprob = Categorical(logits=eta_nn_values) # Logits from policy_old
            log_prob_of_action = policy_dist_for_logprob.log_prob(chosen_action_index)
            
            # ... (Store to self.memory as before) ...
        ...
        else: # Evaluation mode: greedy choice from final refined scores
    ...
    def update_ppo(self):
        ...
        if not self.memory.rewards or len(self.memory.rewards) != len(self.memory.actions):
            ...
            self.memory.clear() # Clear memory to start fresh
            return 0.0, 0.0

        # --- 2. Calculate discounted rewards (advantages) ---
        rewards = []
        discounted_reward = 0
        ...
        # Normalize rewards for the entire batch for better stability
        rewards = (rewards - rewards.mean()) / (rewards.std() + 1e-7)

        # --- 3. Convert other memory data to tensors (ensure they have same length as rewards) ---
        # Using slicing to ensure all tensors have the same length as rewards
        ...
        # --- 4. Optimize policy and value network for K epochs ---
        for _ in range(self.ppo_epochs):
            # We create mini-batches for the update
            indices = np.arange(num_valid_transitions) # Use num_valid_transitions
            ...
            for start_idx in range(0, num_valid_transitions, self.ppo_batch_size):
                ...
                # --- Evaluate old actions with the CURRENT policy on the mini-batch ---
                batch_log_probs, batch_state_values, batch_dist_entropy = [], [], []
                for i in minibatch_indices:
                    # Get features for all frontiers at the state where action i was taken
                ...
                # Get advantages for this mini-batch using the value from the OLD state
                # The critic is also updated, so we want the value estimate from before the update
                # for advantage calculation.
                with torch.no_grad(): # Use no_grad for critic here for advantage calculation
                ...
                # We need to detach minibatch_advantages when calculating policy loss
                # because the policy network (actor) should not try to backpropagate through the value network (critic).
                policy_loss = -torch.min(surr1, surr2).mean()
                
                # Value loss uses the new state values and the calculated returns (rewards)
                value_loss = self.MseLoss(batch_state_values, rewards[minibatch_indices])
                
                # Entropy bonus encourages exploration by making policy less deterministic
        ...
        # Return losses for logging if needed
        # These are from the last mini-batch, not a true average over the epoch, but a good indicator.
