...
import os # For model loading/saving

# Assuming these are in the same 'planners' package or project root is in PYTHONPATH
from .base_planner import BasePlanner 
from environment import UNKNOWN, FREE, OBSTACLE 
try:
    from .geometry_utils import check_line_of_sight # If used for IG feature
except ImportError:
    try:
        ...
        def check_line_of_sight(r1,c1,r2,c2,m): return True # Dummy

# --- Neural Network for Heuristic Estimation (using CNN for local patch) ---
class HeuristicNetworkCNN(nn.Module):
    def __init__(self, patch_side_length, num_input_channels, num_global_features, 
                 cnn_channels1=16, cnn_channels2=32, cnn_fc_out_dim=32, # Reduced cnn_fc_out_dim
        ...
        # with torch.no_grad(): # Calculate flattened size dynamically
    ...
    def forward(self, local_map_patch_batch, global_features_batch): 
        ...
        return F.softplus(heuristic_value) + 1e-6 # Ensure positive output for eta
...
class DeepACOPlanner(BasePlanner):
    # ... (__init__ and other helper methods _get_map_patch_2d, 
    #      _get_global_features_for_heuristic_input, 
    #      _get_global_state_features_for_value_net,
    #      _get_nn_heuristic_for_frontier - as in the previous complete version)
    # ... (Ensure these helpers return NumPy arrays or tensors as expected by their callers)

    def __init__(self, environment,
                 n_ants=8, n_iterations=5, # Reduced defaults for faster iteration
                 ...
                 learning_rate_value=1e-4, # Value net LR often slightly higher
                 gamma_rl=0.99, 
        ...
        self.episode_state_values_for_baseline = [] # Will store TENSORS now
    ...
    def _get_map_patch_2d(self, center_r, center_c, map_data, patch_radius):
        ...
        # Create the patch with the correct number of channels expected by the CNN
        # self.nn_input_channels should be 1 if only known_map is used.
        output_patch = np.full((self.nn_input_channels, patch_side, patch_side), 
                               float(UNKNOWN), dtype=np.float32) # Use float for tensor

        # --- Assuming self.nn_input_channels is 1 (only known_map) ---
        # If you plan to use multiple channels (e.g., known_map + pheromone_map),
        # this logic needs to be expanded to fill each channel of output_patch.
        ...
        # Calculate the slice from the original map_data (which is 2D)
        ...
        # Calculate where this slice fits into the single channel of the output_patch
        ...
        if h_copy > 0 and w_copy > 0:
            output_patch[0, # Fill the first (and only) channel
        ...
        return output_patch # Shape: (num_input_channels, patch_side, patch_side)
    ...
    def _get_nn_heuristic_for_frontier_batch(self, frontier_batch_map_patches_tensor, 
        ...
        # When sampling actions, heuristic_net can be in eval mode if its output is not directly part of policy grad.
        # However, if its output *is* the logits for policy, it should be in train mode.
        # For REINFORCE, the log_probs are derived from these outputs, so it should be in train mode
        # to allow gradients to flow back to the heuristic_net.
        # self.heuristic_net.train() # Set to train mode for action selection if its params are policy params
    ...
    def plan_next_action(self, robot_pos, known_map, **kwargs):
        ...
        batch_map_patches_for_nn_list = [] # Python list of numpy arrays
        batch_global_features_for_nn_list = [] # Python list of numpy arrays
        temp_frontier_path_cost_info_list = [] # Python list of dicts
        ...
        for f_pos_cand in frontiers:
            ...
            if path_to_f:
                ...
                # print(f"Debug: map_patch_np_chw shape: {map_patch_np.shape}") 
                ...
                batch_map_patches_for_nn_list.append(torch.tensor(map_patch_np,dtype=torch.float32)) # Store numpy array
                batch_global_features_for_nn_list.append(global_features_np) # Store numpy array
        ...
        # --- Get NN heuristics for all candidates in one batch ---
        self.heuristic_net.train() # << इंश्योर करें कि पॉलिसी ग्रेडिएंट गणना के लिए train() मोड में है
        ...
        ) # This tensor must have requires_grad=True through heuristic_net
        ...
        for i, f_data in enumerate(temp_frontier_path_cost_info_list):
            candidate_frontiers_with_scores.append({
                ...
                'eta_nn_tensor': eta_nn_values_tensor[i] # Store the TENSOR output for this frontier
            })
        
        # ... (ACO Iterations - same as before, using eta_nn_tensor.item() for scores) ...
        ...
        # --- Robot's Final Target Selection using Softmax on NN Heuristics ---
        if not candidate_frontiers_with_scores: 
             if self.episode_rewards and len(self.episode_log_probs) == len(self.episode_rewards) + 1:
                 self.store_reward_for_last_action(0.0, True)
             return self._final_fallback_plan(robot_pos, known_map)

        # Policy logits are the eta_nn_values_tensor (direct outputs from heuristic_net)
        policy_logits_for_selection = torch.stack([f_dict['eta_nn_tensor'] for f_dict in candidate_frontiers_with_scores])
        
        # Use ACO scores for actual selection by robot if q0 is not 1 (i.e., if ACO pheromones play a role in robot's final choice beyond just ants)
        # For REINFORCE, the log_prob must be of the action taken based on the *policy network's output distribution*.
        # So, the robot should sample based on policy_logits_for_selection if we want to train heuristic_net as a policy.
        ...
        try:
            if torch.isnan(action_probs_from_nn_policy).any() or torch.isinf(action_probs_from_nn_policy).any() or action_probs_from_nn_policy.sum().item() < 1e-6 :
                # print("Warning: Invalid policy probabilities for robot choice. Choosing based on max eta_nn or random.")
            ...
            # print(f"Warning: torch.multinomial error on policy probs for robot. Choosing based on max eta_nn or random.")
            if candidate_frontiers_with_scores:
                eta_nn_items = [f['eta_nn_tensor'].item() for f in candidate_frontiers_with_scores]
                if any(s > 0 for s in eta_nn_items): chosen_idx_by_robot = np.argmax(eta_nn_items)
                else: chosen_idx_by_robot = random.randrange(len(candidate_frontiers_with_scores))
            else: # Should not be reached if caught earlier
        ...
        # Store log_prob of the action chosen by the policy network
        ...
        # *** CRITICAL CHANGE FOR VALUE NET GRADIENT ***
        self.value_net.train() # Set value_net to train mode BEFORE forward pass for V(St)
        state_value_tensor_for_baseline = self.value_net(current_global_state_for_value_net_tensor).squeeze() 
        # DO NOT call .item() or use torch.no_grad() here if this tensor is used in loss calculation for value_net
    ...
    def store_reward_for_last_action(self, reward, done):
        if self.episode_log_probs : 
             ...
             # If done, the episode_log_probs, rewards, state_values will be used for training then cleared.

    # --- finish_episode_and_train (same as before) ---
    def finish_episode_and_train(self, final_reward_for_terminal_state=0.0):
        ...
        if not self.episode_log_probs or not self.episode_rewards or \
           ...
           len(self.episode_state_values_for_baseline) != len(self.episode_rewards) :
            # print(f"Warning: Data mismatch or no data for training. "
            #       f"LogP: {len(self.episode_log_probs)}, Rew: {len(self.episode_rewards)}, V_base: {len(self.episode_state_values_for_baseline)}")
            self.episode_log_probs = []; self.episode_rewards = []; self.episode_state_values_for_baseline = []
            return 0.0, 0.0 # No training happened
        ...
        current_return_g = 0.0 # If episode ended, final state's value is 0 unless specified otherwise
        # If last stored reward was for a non-terminal step, the G for it is just that reward (if gamma=0 for last step)
        # Or, if we assume a final_reward_for_terminal_state for a state *after* the last action.
        # For REINFORCE, G_t = R_{t+1} + gamma*R_{t+2} + ...
        # So, G_{T-1} = R_T. If episode ends at T, R_T is the reward for action A_{T-1}.
        # The final_reward_for_terminal_state is if we consider a virtual step after T.
        # Let's simplify: G_t is sum of rewards from t to end.
        
        # Correct G_t calculation:
        # G_T-1 = R_T
        # G_T-2 = R_T-1 + gamma * R_T (this R_T is actually G_T-1 based on formula)
        # G_t = R_{t+1} + gamma * G_{t+1}
        
        # Let's assume self.episode_rewards are R_1, R_2, ..., R_T
        # where R_k is reward after action A_{k-1} (from state S_{k-1})
        # log_probs are log P(A_0|S_0), log P(A_1|S_1), ..., log P(A_{T-1}|S_{T-1})
        # state_values are V(S_0), V(S_1), ..., V(S_{T-1})
        ...
        # G_t = r_t + gamma * r_{t+1} + gamma^2 * r_{t+2} + ... (using r_t as reward for action a_t)
        # Or, if R_k is reward for action A_{k-1}: G_t = R_{t+1} + gamma*R_{t+2} + ...
        # The self.episode_rewards stores R_1, R_2, ..., R_T
        # G[i] should be sum of gamma^k * R[i+k+1]
        
        # Correct calculation for G_t = sum_{k=0}^{T-1-t} gamma^k * R_{t+k+1}
        # returns_g_tensor should be same length as log_probs and state_values
        ...
        if log_probs.ndim == 0: log_probs = log_probs.unsqueeze(0) # Handle single step episode
        if advantages.ndim == 0: advantages = advantages.unsqueeze(0)
        
        policy_loss = (-log_probs * advantages).mean() 
        
        if state_values.ndim == 0: state_values = state_values.unsqueeze(0) # Handle single step for MSE
        ...
        # else: print("Warning: Policy loss does not require grad.")

        self.optimizer_value.zero_grad()
        if value_loss.requires_grad: # Should always be true if state_values has grad
            value_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.value_net.parameters(), 1.0) 
            self.optimizer_value.step()
        # else: print("Warning: Value loss does not require grad.")
    ...
    # --- save_model, load_model (same as before) ---
